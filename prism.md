...
3. **Voz.** Copia la carpeta `voice/` de este repo dentro del hogar. Comprueba si `uv` está instalado (`uv --version`); si no, instálalo con permiso de la persona ("una herramienta que descarga el Python correcto sola, sin que tengas que instalar nada a mano"). Crea el entorno con `uv sync` dentro de `voice/` (usa `voice/pyproject.toml`, que fija Python `>=3.11,<3.13` a propósito) — **nunca uses `python3 -m venv` + `pip` a pelo aquí**: Python 3.13 hace que `kokoro` arrastre `spacy`→`thinc`→`blis`, y esa cadena no tiene wheel precompilado para 3.13 en ARM64, así que intenta compilar desde código fuente y falla (verificado durante la instalación de prueba del 2026-09-10). Escribe `voice.json` con el motor elegido en la Fase 2, usando `VoiceConfig` como formato. **La voz de Kokoro tiene que hablar el idioma elegido en la Fase 1, nunca inglés por defecto** — Kokoro nombra sus voces por idioma (primera letra: `b`=inglés británico, `a`=inglés americano, `e`=español, ...); para español usa `em_alex`, `ef_dora` o `em_santa` (confirma en `huggingface.co/hexgrad/Kokoro-82M` si aparece alguna más nueva), nunca dejes el default de `voice/config.py` (`bm_lewis`, inglés) sin corregir si la persona no eligió inglés — verificado en la instalación de prueba: con una voz del idioma equivocado, el habla es prácticamente ininteligible aunque el motor funcione bien.
